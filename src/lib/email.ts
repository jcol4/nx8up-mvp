...
function buildEmailHtml({ title, message, link }: Pick<SendEmailInput, 'title' | 'message' | 'link'>): string {
  const ctaButton = link
    ? `<a href="${link}" style="display:inline-block;margin-top:20px;padding:10px 20px;background:#00c8ff;color:#000;font-size:0.9375rem;font-weight:bold;border-radius:6px;text-decoration:none;">View Details</a>`
    : ''
...
<body style="font-family:sans-serif;font-size:108%;background:#060d18;color:#c8dff0;padding:32px;">
  <div style="max-width:480px;margin:0 auto;background:#0d1d30;border-radius:12px;padding:24px;border:1px solid rgba(0,200,255,0.15);">
    <p style="color:#00c8ff;font-size:0.75rem;font-weight:bold;letter-spacing:0.1em;text-transform:uppercase;margin:0 0 8px;">NX8UP Notification</p>
    <h2 style="color:#e8f4ff;margin:0 0 12px;font-size:1.25rem;">${title}</h2>
    <p style="color:#8aa4bf;margin:0;line-height:1.6;font-size:1rem;">${message}</p>
    ${ctaButton}
  </div>
  <p style="text-align:center;margin-top:24px;font-size:0.75rem;color:#4a6080;">
