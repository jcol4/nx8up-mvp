 ...
 * - Max 100 creators returned (pre-filter, sorted by Twitch followers server-side;
 *   client-side re-sort by combined followers is applied after).
 * - Columns: rank, handle/location/categories, platforms (incl. Steam badge if
 *   linked), followers (split by platform), avg views, CTR (color-coded:
 *   green ≥5%, yellow ≥2%), content type, creator size.
 * - When a creator has linked their Steam account, the Steam username and
 *   SteamID are surfaced under their handle so sponsors can verify identity.
 *
 * Gotcha: The `take: 100` and `orderBy: { subs_followers: 'desc' }` on the Prisma
 * query means the server-side sort is Twitch-only. If a YouTube-only creator has
 * more total followers than a Twitch creator at rank ~100, they may be excluded.
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        {c.steam_id && (
                          <p className="text-[11px] dash-text-muted mt-0.5">
                            <span className="text-[#66c0f4]">Steam:</span>{' '}
                            {c.steam_username ?? 'Connected'}
                            <span className="font-mono ml-1 opacity-70">({c.steam_id})</span>
                          </p>
                        )}
                        {c.game_category.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          ...
                          {c.steam_id && (
                            <span className="text-[11px] px-1.5 py-0.5 rounded bg-[#66c0f4]/10 text-[#66c0f4]">Steam</span>
                          )}
                        </div>
                      </td>
