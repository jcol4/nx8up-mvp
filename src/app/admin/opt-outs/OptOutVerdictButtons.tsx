...
import { submitOptOutVerdict } from './_actions'

type Props = { optOutId: string }

export default function OptOutVerdictButtons({ optOutId }: Props) {
  ...
  const submit = (verdict: 'valid' | 'invalid') => {
    ...
    startTransition(async () => {
      const res = await submitOptOutVerdict(optOutId, verdict, notes || undefined)
      if (res.error) {
        setError(res.error)
      } else {
        router.refresh()
  ...
  return (
    <div className="mt-3 space-y-2 border-t border-white/10 pt-3">
      ...
      <div className="flex gap-2">
        ...
        >
          {isPending ? '…' : 'Invalid reason (-10 pts)'}
        </button>
        <button
          type="button"
