...
export default function SponsorProfileForm({ profile }: Props) {
  const tr = useTranslations('sponsor.profileForm')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (ageRestrictionChanged) {
      setError(tr('errAgeRestriction'))
      return
    }
    if (budgetMinOver || budgetMaxOver) {
      setError(tr('errBudget', { max: format.number(BUDGET_MAX) }))
      return
    }
    setError('')
    setIsSaving(true)
  ...
  return (
    <>
      ...
      <div className={sectionClass}>
        <p className={sectionTitle}>{tr('profilePicture')}</p>
        <p className="mb-3 text-xs cr-text-muted">
          {tr('profilePictureDesc')}
        </p>
      ...
      {/* ── Company Info ─────────────────────────────────────────────── */}
      <div className={sectionClass}>
        <p className={sectionTitle}>{tr('companyInfo')}</p>

        <div>
          <label htmlFor="company-name" className={labelClass}>{tr('companyName')}</label>
          <FormInput
            id="company-name"
            type="text"
            variant="dashboard"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="country" className={labelClass}>{tr('country')}</label>
            <FormSelect
              id="country"
              variant="dashboard"
              value={country}
            ...
            >
              <option value="">{tr('countryDefault')}</option>
              {COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
            </FormSelect>
          </div>
          {stateOptionsForCountry(country).length > 0 && (
            <div>
              <label htmlFor="state" className={labelClass}>{tr('state')}</label>
              <FormSelect
                id="state"
                variant="dashboard"
                value={state}
              ...
              >
                <option value="">{tr('stateDefault')}</option>
                {stateOptionsForCountry(country).map(s => <option key={s} value={s}>{s}</option>)}
              </FormSelect>
            </div>
          )}
          <div>
            <label htmlFor="city" className={labelClass}>{tr('city')}</label>
            <FormInput
              id="city"
              type="text"
              variant="dashboard"
        ...
        <div>
          <label className={labelClass}>{tr('languages')}</label>
          <p className="mb-2 text-xs cr-text-muted">{tr('languagesDesc')}</p>
      ...
      <div className={sectionClass}>
        <p className={sectionTitle}>{tr('campaignPreferences')}</p>

        <div>
          <label className={labelClass}>{tr('preferredPlatforms')}</label>
          <p className="mb-2 text-xs cr-text-muted">{tr('preferredPlatformsDesc')}</p>
          <div className="flex flex-wrap gap-2">
            {PLATFORMS.map(p => (
              <button
                key={p}
        ...
        <div>
          <label className={labelClass}>{tr('contentTypes')}</label>
          <p className="mb-2 text-xs cr-text-muted">{tr('contentTypesDesc')}</p>
          <div className="flex flex-wrap gap-2">
        ...
        <div>
          <label className={labelClass}>{tr('gameFocus')}</label>
          ...
          <div className="flex gap-2">
            <FormInput
              ...
              placeholder={tr('gamePlaceholder')}
              className="flex-1"
            />
            <button
              type="button"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="budget-min" className={labelClass}>{tr('budgetMin')}</label>
            <FormInput
              id="budget-min"
              type="text"
              inputMode="numeric"
              ...
              placeholder={tr('budgetMinPlaceholder')}
            />
            {budgetMinOver && (
              <div className="mt-1.5 flex items-start gap-1.5 p-2.5 rounded-lg border border-amber-500/40 bg-amber-500/8">
                ...
                <p className="text-xs text-amber-400">{tr('budgetOverMax', { max: format.number(BUDGET_MAX) })}</p>
              </div>
            )}
          </div>
          <div>
            <label htmlFor="budget-max" className={labelClass}>{tr('budgetMax')}</label>
            <FormInput
              id="budget-max"
              type="text"
              inputMode="numeric"
              ...
              placeholder={tr('budgetMaxPlaceholder')}
            />
            {budgetMaxOver && (
              <div className="mt-1.5 flex items-start gap-1.5 p-2.5 rounded-lg border border-amber-500/40 bg-amber-500/8">
                <svg className="w-3.5 h-3.5 shrink-0 mt-0.5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* ── Payment Preferences ──────────────────────────────────────── */}
      <div className={sectionClass}>
        <p className={sectionTitle}>{tr('paymentPreferences')}</p>
        <p className="mb-3 text-xs cr-text-muted">
          {tr('paymentPreferencesDesc')}
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {([
            { value: 'card', label: tr('methodCard'), desc: tr('methodCardDesc') },
            { value: 'ach',  label: tr('methodAch'),  desc: tr('methodAchDesc') },
            { value: 'both', label: tr('methodBoth'), desc: tr('methodBothDesc') },
          ] as const).map(opt => {
        ...
        {(preferredPaymentMethod === 'ach' || preferredPaymentMethod === 'both') && (
          <div className="mt-3 flex items-start gap-2 p-3 rounded-lg border border-red-500/30 bg-red-500/5">
            ...
            <p className="text-xs text-red-400 leading-relaxed">
              <span className="font-semibold">{tr('achWarnTitle')}</span> {tr('achWarnDesc')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className={sectionClass}>
        <p className={sectionTitle}>
          {tr('creatorReqs')}{' '}
          <span className="normal-case font-normal">{tr('optional')}</span>
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="min-avg-viewers" className={labelClass}>{tr('minAvgViewers')}</label>
            <FormInput
              id="min-avg-viewers"
              type="text"
              inputMode="numeric"
              ...
              placeholder={tr('minAvgViewersPlaceholder')}
            />
          </div>
          <div>
            <label htmlFor="min-subs" className={labelClass}>{tr('minSubs')}</label>
            <FormInput
              id="min-subs"
              type="text"
              inputMode="numeric"
              ...
              placeholder={tr('minSubsPlaceholder')}
            />
          </div>
          <div>
            <label htmlFor="min-engagement" className={labelClass}>{tr('minCtr')}</label>
            <FormInput
              id="min-engagement"
              type="text"
              inputMode="decimal"
              ...
              placeholder={tr('minCtrPlaceholder')}
            />
          </div>
        </div>
      </div>
      ...
      <div className={sectionClass}>
        <div>
          <p className={sectionTitle}>{tr('ageRestrictions')}</p>
          <p className="mt-1 mb-5 text-xs cr-text-muted">
            {tr('ageRestrictionsDesc')}
          </p>
          ...
          {ageRestricted && (
            <div className="mt-3 ml-7 space-y-2">
              <p className="mb-2 text-xs cr-text-muted">{tr('ageMinLabel')}</p>
          ...
          <div className="ml-0 space-y-3 rounded-lg border border-[#99f7ff]/20 bg-[#99f7ff]/5 p-3">
            ...
            <div>
              <label className={labelClass}>
                {tr('reasonForChange')} <span className="text-red-400">*</span>
              </label>
              <textarea
              ...
              />
              <p className="mt-0.5 text-right text-xs cr-text-muted">{tr('charCount', { n: ageChangeMessage.length })}</p>
            </div>
            {ageChangeError && <p className="text-xs text-red-400">{ageChangeError}</p>}
            {ageChangeSuccess && <p className="text-xs text-[#22c55e]">{tr('changeSubmitted')}</p>}
            <button
              type="button"
            ...
            >
              {isSubmittingAgeChange ? tr('submittingChange') : tr('submitChangeRequest')}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-3">
          ...
          >
            {isSaving ? tr('saving') : tr('saveProfile')}
          </button>
          <button
            type="button"
