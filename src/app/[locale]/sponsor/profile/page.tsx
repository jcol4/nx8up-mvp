...
export default async function SponsorProfilePage() {
  ...
  const t = await getTranslations('sponsor.profile')
          ...
          {profile && (() => {
            ...
            return (
              <div className={`flex items-start gap-3 rounded-xl border p-4 ${tierColors[tier]}`}>
                ...
                <div>
                  <p className="text-sm font-semibold">{t('tierSponsor', { tier: tEnum(`tier.${tier}`) })}</p>
                  <p className="mt-0.5 text-sm cr-text-muted">{tEnum(`tierDesc.${tier}`)}</p>
                </div>
              </div>
            )
          })()}
