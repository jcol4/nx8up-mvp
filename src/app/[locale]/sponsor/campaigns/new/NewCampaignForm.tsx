...
function validateStep(step: number, draft: CampaignDraft): string {
  switch (step) {
    case 1:
      if (!draft.title.trim()) return 'valTitle'
      if (!draft.product_type) return 'valProductType'
      if (!draft.objective) return 'valObjective'
      if (!draft.platform.length) return 'valPlatform'
      return ''
    case 3:
      if (draft.is_direct_invite && !draft.invited_creator_id) return 'valInviteCreator'
      return ''
    case 4:
      if (!draft.budget || parseInt(draft.budget, 10) <= 0) return 'valBudget'
      if (!draft.is_direct_invite && (!draft.creator_count || parseInt(draft.creator_count, 10) <= 0)) return 'valCreatorCount'
      if (!draft.start_date) return 'valStartDate'
      if (!draft.end_date) return 'valEndDate'
      if (draft.start_date >= draft.end_date) return 'valEndAfterStart'
      return ''
    case 5:
      if (!draft.campaign_type) return 'valCampaignType'
      return ''
    default:
      return ''
  }
...
export default function NewCampaignForm({ initialDraft, editingId, sponsorAgeRestriction, sponsorReputationTier = 'neutral', availableCreators = [] }: Props) {
  ...
  const goNext = () => {
    ...
    if (err) { setStepError(t(err)); return }
    setStepError('')
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-[#e8f4ff]">
            {t('stepHeading', { n: step, label: STEP_LABELS[step - 1] })}
          </h2>
          <p className="text-xs text-[#a9abb5] mt-0.5">{t('progress', { n: step, total: TOTAL_STEPS })}</p>
        </div>
        <button
          type="button"
        ...
        >
          {draftSaved ? (
            <>
              ...
              <span className="text-[#22c55e]">{t('saved')}</span>
            </>
          ) : (
            <>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
