...
export default async function CampaignPayPage({
  ...
  const t = await getTranslations('sponsor.pay')
    ...
    <>
      ...
      <div className="flex-1 p-6 overflow-auto">
        <div className="max-w-lg mx-auto space-y-6">
          ...
          <div>
            <h1 className="text-xl font-bold dash-text-bright">{t('title')}</h1>
            <p className="text-sm dash-text-muted mt-1">{campaign.title}</p>
            <p className="text-xs dash-text-muted mt-0.5">
              {t.rich('invoiceNotice', {
                email: sponsor.email,
                b: (chunks) => <span className="dash-text">{chunks}</span>,
              })}
            </p>
          </div>
          ...
          <div className="dash-panel dash-panel--nx-top p-4 space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="dash-text-muted">{t('creatorPool')}</span>
              <span className="text-[#22c55e] font-semibold">${format.number(creatorPool)}</span>
            </div>
            <div className="flex justify-between">
              <span className="dash-text-muted">{t('platformFee', { pct: Math.round(NX_FEE_RATE * 100) })}</span>
              <span className="text-red-400/70">${format.number(fee)}</span>
            </div>
            <div className="flex justify-between font-semibold border-t border-white/10 pt-2">
              <span className="dash-text-muted">{t('totalDue')}</span>
              <span className="dash-text-bright">${format.number(campaign.budget)}</span>
            </div>
            {perCreator && (
              <p className="text-xs dash-text-muted pt-1 border-t border-white/5">
                {t.rich('perCreator', {
                  amount: `$${format.number(perCreator)}`,
                  n: campaign.creator_count ?? 0,
                  b: (chunks) => <span className="text-[#22c55e] font-medium">{chunks}</span>,
                })}
              </p>
            )}
          </div>
              ...
              return (
                <div className="flex items-start gap-2.5 rounded-lg border border-red-500/25 bg-red-500/10 p-3">
                  ...
                  <p className="text-xs text-red-300">
                    {t.rich('sanctionedNotice', { b: (chunks) => <span className="font-semibold">{chunks}</span> })}
                  </p>
                </div>
              )
            }
          ...
          <PaymentForm
            ...
            campaignTitle={campaign.title}
            budgetDisplay={`$${format.number(campaign.budget)}`}
            returnUrl={returnUrl}
