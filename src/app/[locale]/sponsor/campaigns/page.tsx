...
const STATUS_LABEL_KEY: Record<string, string> = {
  ...
  payment_in_progress: 'statusPaymentInProgress',
...
export default async function SponsorCampaignsPage({
  ...
  const t = await getTranslations('sponsor.campaigns')
        ...
        <div className="mx-auto max-w-4xl">
          {payment === 'processing' && (
            <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#eab308]/30 bg-[#eab308]/5 p-4">
              ...
              <div>
                <p className="text-sm font-semibold text-[#eab308]">{t('achInitiated')}</p>
              ...
              return (
                <div className="mb-4 flex items-start gap-2.5 rounded-lg border border-red-500/25 bg-red-500/10 p-3">
                  ...
                  <p className="text-xs text-red-300">
                    {t('sanctionedBefore')}<span className="font-semibold">{t('sanctionedWord')}</span>{t('sanctionedAfter')}
                  </p>
                </div>
              )
            }
              ...
              return (
                <div className="mb-4 flex items-start gap-2.5 rounded-lg border border-[#99f7ff]/20 bg-[#99f7ff]/5 p-3">
                  ...
                  <p className="text-xs text-[#99f7ff]/80">
                    {t.rich('cooldownNotice', {
                      tier: tEnum(`tier.${tier}`),
                      n: cooldown,
                      b: (chunks) => <span className="font-semibold text-[#99f7ff]">{chunks}</span>,
                    })}
                  </p>
                </div>
              )
          ...
          {campaigns.length === 0 ? (
            <div className="dash-panel dash-panel--nx-top p-8 text-center text-[#a9abb5]">
              {activeTab === 'launched' ? (
                <>
                  <p className="mb-2 text-[#e8f4ff]">{t('emptyLaunched')}</p>
                ...
                <>
                  <p className="mb-4 text-[#e8f4ff]">{t('emptyActive')}</p>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        <div className="mb-1 flex flex-wrap items-center gap-2">
                          ...
                          >
                            {STATUS_LABEL_KEY[c.status] ? t(STATUS_LABEL_KEY[c.status]) : c.status}
                          </span>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          {c.objective && (
                            <span className={chipBase}>{tEnum.has(`objective.${c.objective}.label`) ? tEnum(`objective.${c.objective}.label`) : c.objective}</span>
                          )}
                          {c.campaign_type && (
                            <span className={chipBase}>{tEnum.has(`campaignFormat.${c.campaign_type}`) ? tEnum(`campaignFormat.${c.campaign_type}`) : c.campaign_type}</span>
                          )}
                          {c.payment_model && (
                            <span className={chipBase}>{tEnum.has(`paymentModel.${c.payment_model}`) ? tEnum(`paymentModel.${c.payment_model}`) : c.payment_model}</span>
                          )}
                          {c.budget != null && (
                          ...
                          {c.end_date && (
                            <span className={`${chipBase} text-[#a9abb5]`}>
                              {t('ends', { date: format.dateTime(new Date(c.end_date), 'numeric') })}
                            </span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex shrink-0 flex-wrap items-center justify-end gap-2 sm:gap-3">
                        ...
                        {c._count.applications > 0 ? (
                          ...
                          <span className="text-sm text-[#6b7280]">{t('noApplicants')}</span>
                        )}
                        {c.status === 'live' ? (
                          <RequestRefundButton
                            campaignId={c.id}
