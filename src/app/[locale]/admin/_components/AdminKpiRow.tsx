import { getAdminKpisCached } from '@/lib/admin-dashboard-cache'
...
function KpiCard({
  ...
  return (
    <div className="glass-panel interactive-panel rounded-xl border border-white/10 border-t-2 border-t-[#99f7ff] p-4 neon-glow-teal">
      ...
      <span className={`mt-3 inline-flex rounded-md border px-2 py-0.5 text-nx-11 font-medium ${toneMap[tone]}`}>
        {hint}
      </span>
    </div>
  )
...
export default async function AdminKpiRow() {
  const { campaigns, submittedDeals, creators, sponsors } = await getAdminKpisCached()
  ...
  return (
    <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <KpiCard label="Active Creators" value={creators.toLocaleString()} hint="Eligible to apply" tone="cyan" />
