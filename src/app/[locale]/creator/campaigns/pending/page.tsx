      ...
      <CreatorShell>
        <main className="creator-campaigns mx-auto max-w-5xl space-y-6 p-6 sm:p-8">
          <div className="rounded-xl border border-white/10 bg-black/20 p-5">
            ...
            <h1 className="mt-1 font-headline text-xl font-semibold text-[#e8f4ff]">{t('pendingTitle')}</h1>
            <p className="mt-2 text-sm cr-text-muted">{t('errorLoadProfile')}</p>
          </div>
        </main>
      </CreatorShell>
    )
    ...
    <CreatorShell>
      <main className="creator-campaigns mx-auto max-w-5xl space-y-6 p-6 sm:p-8">
        <div className="rounded-xl border border-white/10 bg-black/20 p-5">
          <p className="font-headline text-nx-11 uppercase tracking-[0.2em] text-[#99f7ff]">{t('heading')}</p>
          <h1 className="mt-1 font-headline text-xl font-semibold text-[#e8f4ff]">{t('pendingTitle')}</h1>
          <p className="mt-1 text-sm cr-text-muted">{t('pendingDesc')}</p>
        </div>
        ...
        {applications.length === 0 ? (
          <div className="glass-panel interactive-panel rounded-xl border border-white/10 border-t-2 border-t-[#99f7ff] p-8 text-center">
            <p className="text-sm cr-text-muted">{t('emptyPending')}</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs cr-text-muted">
                        ...
                        {app.campaign.end_date ? ` · ${t('deadline')} ${format.dateTime(new Date(app.campaign.end_date), 'numeric')}` : ''}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      <p className="text-sm font-semibold text-[#99f7ff]">
                        {app.campaign.budget != null ? `$${format.number(app.campaign.budget)}` : '—'}
                      </p>
                      <p className="text-nx-10 cr-text-muted">{t('campaignBudget')}</p>
                    </div>
                  </div>
                </Link>
              </li>
