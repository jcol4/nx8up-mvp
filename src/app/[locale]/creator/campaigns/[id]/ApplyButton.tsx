  ...
  if (!eligible) {
    return (
      <div className="flex items-center justify-between gap-6">
        <div>
          <h2 className="text-base font-semibold cr-text-bright">{t('applyHeading')}</h2>
          <p className="text-xs cr-text-muted mt-0.5">{t('applyDesc')}</p>
        </div>
        ...
        {legalAgeRestriction && (
          <div className="flex items-start gap-2.5 p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
            ...
            <p className="text-xs text-orange-400 leading-relaxed">
              <span className="font-semibold">{t('applyLegalNotice', { restriction: legalAgeRestriction })}</span>{' '}
              {t('applyLegalWarning', { age: legalAgeRestriction === '21+' ? '21' : '18' })}
            </p>
          </div>
        )}
        <div className="flex items-center justify-between gap-6">
    ...
    <div className="space-y-4">
      ...
      {legalAgeRestriction && (
        <div className="flex items-start gap-2.5 p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
          ...
          <p className="text-xs text-orange-400 leading-relaxed">
            <span className="font-semibold">{t('applyLegalNotice', { restriction: legalAgeRestriction })}</span>{' '}
            {t('applyLegalWarningFull', { age: legalAgeRestriction === '21+' ? '21' : '18' })}
          </p>
        </div>
      )}
      ...
      {/* Pitch */}
      <div>
        <label className={labelClass}>{t('applyPitchLabel')}</label>
        <textarea
          value={message}
          ...
          maxLength={1000}
        />
        <p className="text-xs cr-text-muted mt-1 text-right">{t('applyCharCount', { n: message.length })}</p>
      </div>

      {/* Location */}
      <div>
        <label className={labelClass}>{t('applyLocationLabel')}</label>
        <input
          type="text"
          value={location}
          onChange={(e) => setLocation(e.target.value)}
          placeholder={t('applyLocationPlaceholder')}
      ...
      {/* Audience age range */}
      <div>
        <label className={labelClass}>{t('applyAudienceAgeLabel')}</label>
        <div className="flex items-center gap-3">
          <input
            type="number"
            value={ageMin}
            onChange={(e) => setAgeMin(e.target.value)}
            placeholder={t('applyAgeMin')}
            min={13}
            max={65}
            className={`${inputClass} w-24`}
          />
          <span className="cr-text-muted text-sm shrink-0">{t('applyAgeTo')}</span>
          <input
            type="number"
            value={ageMax}
            onChange={(e) => setAgeMax(e.target.value)}
            placeholder={t('applyAgeMax')}
            min={13}
            max={65}
            className={`${inputClass} w-24`}
          />
      ...
      {/* Audience locations */}
      <div>
        <label className={labelClass}>{t('applyAudienceLocationLabel')}</label>
      ...
      {acceptedMediaTypes.length > 0 && (
        <div>
          <label className={labelClass}>{t('applyContentTypeLabel')}</label>
          <div className="flex flex-wrap gap-2">
