...
export default function CreatorCommandCenter({
  ...
  const collapsedNavItems: SidebarNavItem[] = [
    { href: '/creator', label: tNav('dashboard'), icon: 'dashboard', exact: true },
    { href: '/creator/profile', label: tNav('profile'), icon: 'profile' },
    { href: '/creator/campaigns', label: tNav('campaigns'), icon: 'campaigns' },
    { href: '/creator/deal-room', label: tNav('dealRoom'), icon: 'dealRoom' },
    { href: '/creator/academy', label: tNav('academy'), icon: 'academy' },
    { href: '/creator/steam-lookup', label: tNav('steamLookup'), icon: 'creators' },
    { href: '/creator/settings/notifications', label: tNav('notifications'), icon: 'notifications' },
  ]
  const navGroups: SidebarNavGroup[] = [
      ...
      : []),
    {
      ...
      items: [
        ...
        { href: '/creator/deal-room', label: tNav('dealRoom'), icon: 'dealRoom' },
        { href: '/creator/academy', label: tNav('academy'), icon: 'academy' },
        { href: '/creator/guide', label: tNav('guide'), icon: 'reports' },
        { href: '/creator/steam-lookup', label: tNav('steamLookup'), icon: 'creators' },
      ],
    },
    {
      title: t('notificationsGroupTitle'),
      borderTop: true,
      items: [
        { href: '/creator/settings/notifications', label: tNav('preferences'), icon: 'notifications' },
      ],
    },
  ]
  ...
  const statsRows = [
    ...
    },
    {
      ...
      value: creatorStats.averageVodViews != null ? format.number(creatorStats.averageVodViews) : '—',
    },
    {
      label: t('statsTwitch'),
      value: creatorStats.twitchUsername ? `@${creatorStats.twitchUsername}` : t('statsTwitchEmpty'),
    ...
    {
      label: t('statsYoutube'),
      value: creatorStats.youtubeChannelName ? `@${creatorStats.youtubeChannelName}` : t('statsYoutubeEmpty'),
  ...
  const monthLabel = format.dateTime(viewDate, 'monthYear').toUpperCase()
  const weekStart = startOfWeek(selectedDate)
  const weekLabel = t('weekOf', {
    date: format.dateTime(weekStart, 'monthDay').toUpperCase(),
  })
        ...
        {(collapsed) => (
          <>
            ...
            <main className="space-y-6 p-6 pt-24">
              ...
              <div data-reveal className="reveal-item grid grid-cols-1 gap-6 lg:grid-cols-3">
                <section className="glass-panel interactive-panel rounded-xl border border-white/10 border-t-2 border-t-[#99f7ff] p-5 neon-glow-teal">
                  <div className="mb-5 flex items-center justify-between">
                    <div className="mr-4 flex min-w-0 flex-1 items-center gap-3">
                      <h2 className="shrink-0 font-headline text-base tracking-[0.24em] text-[#99f7ff] font-semibold">{t('todaysMissions')}</h2>
                      <div className="group relative shrink-0">
                        <button
                          type="button"
                          aria-label={t('missionsAbout')}
                    ...
                    <span className="text-nx-10 font-medium uppercase tracking-widest text-white/90">{t('activeCount', { n: displayedMissions.filter((m) => !m.completed).length })}</span>
                  </div>
                        ...
                        <div key={mission.id} className="space-y-2 transition-transform duration-200 hover:translate-x-1">
                          <div className="flex items-center justify-between gap-3 text-nx-10">
                            <div className="min-w-0">
                              ...
                              <p className="text-[#99f7ff]">{t('xpLabel', { n: mission.xp })}</p>
                            </div>
                            {mission.completed ? (
                              <span className="shrink-0 rounded border border-[#22c55e]/30 bg-[#22c55e]/10 px-2 py-1 text-nx-9 uppercase tracking-widest text-[#22c55e]">{t('done')}</span>
                            ) : (
                              <span className="shrink-0 rounded border border-white/25 bg-white/[0.04] px-2 py-1 text-nx-9 font-medium uppercase tracking-widest text-white/90">{t('inProgress')}</span>
                            )}
                          </div>
                  ...
                  <div className="mt-6 border-t border-white/10 pt-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-nx-10 font-medium uppercase tracking-widest text-white/90">{t('levelHeading', { n: level })}</p>
                        <p className="font-headline text-sm font-bold tracking-widest text-white">{rankName.toUpperCase()}</p>
                        <p className="mt-1 text-nx-10 font-medium text-white/90">{t('xpProgress', { xp: format.number(xp), xpForNext: format.number(xpForNext) })}</p>
                      </div>
                ...
                <section className="glass-panel interactive-panel rounded-xl border border-white/10 border-t-2 border-t-[#99f7ff] p-5 lg:col-span-2 neon-glow-teal">
                  <div className="mb-5 flex items-center justify-between">
                    <div className="mr-4 flex min-w-0 flex-1 items-center gap-3">
                      <h2 className="shrink-0 font-headline text-base tracking-[0.24em] text-[#99f7ff] font-semibold">{t('campaignsSection')}</h2>
                      <div className="group relative shrink-0">
                        <button
                          type="button"
                          aria-label={t('campaignsAbout')}
                          className="inline-flex h-5 w-5 items-center justify-center rounded-full border border-[#99f7ff]/30 text-[#99f7ff]/80 transition hover:border-[#99f7ff]/55 hover:text-[#99f7ff] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#99f7ff]/50"
                        >
                          <Info className="h-3.5 w-3.5" />
                        </button>
                  ...
                  {displayedCampaigns.length === 0 ? (
                    <div className="flex min-h-56 items-center justify-center rounded-lg border border-white/10 bg-black/20 text-center">
                      <div className="space-y-2">
                        <p className="text-base font-medium text-white/90">{t('emptyCampaigns', { tab: tabLabel })}</p>
                ...
                <div className={`${calendarView === 'month' ? 'mb-6' : 'mb-8'} flex flex-col items-start justify-between gap-4 md:flex-row md:items-center`}>
                  <div className="w-full md:flex-1">
                    <div className="mb-1 flex w-full items-center gap-3">
                      <h2 className="shrink-0 font-headline text-2xl font-bold tracking-tight text-[#99f7ff]">{t('calendarSection')}</h2>
                ...
                <div className="mt-2 grid gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
                  <div>
                    <CalendarGrid
                      ...
                      todayLabel={t('today')}
                      noteLabel={t('noteBadge')}
                      formatTasksScheduled={(n) => t('tasksScheduled', { n })}
                    />
                  </div>
                  <aside className="space-y-3">
                    <div className="rounded-lg border border-[#99f7ff]/30 bg-black/30 p-4">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="mt-1 text-nx-10 uppercase tracking-[0.2em] text-white">{t('dayDetailLog')}</p>
                        </div>
                    ...
                    <div className="rounded-lg border border-white/10 bg-black/25 p-3">
                      <p className="mb-2 text-nx-10 font-medium uppercase tracking-[0.18em] text-white">{t('addTask')}</p>
                      <textarea
                        value={noteDraft}
                    ...
                    <div className="calendar-notes-scroll max-h-[360px] space-y-2 overflow-y-auto rounded-lg border border-white/10 bg-slate-950/70 p-2 pr-1">
                      ...
                      {selectedDayNotes.length > 0 ? (
                          ...
                          <div key={note.id} className="group rounded-lg border border-white/10 bg-white/[0.03] p-3">
                            ...
                            <div className="flex items-start gap-3">
                              <button
                                ...
                                aria-label={note.done ? t('markNoteOpen') : t('markNoteDone')}
                              >
                        ...
                        <div className="rounded-lg border border-dashed border-white/10 bg-black/20 p-3">
                          <p className="text-xs text-slate-500">{t('emptyDay')}</p>
                        </div>
                      ) : null}
                    </div>
                  </aside>
              ...
              <section data-reveal className="reveal-item glass-panel interactive-panel rounded-xl border border-white/10 border-t-2 border-t-[#99f7ff] p-5 neon-glow-teal">
                <div className="mb-5 flex items-center gap-3">
                  <h2 className="shrink-0 font-headline text-sm tracking-[0.22em] text-[#99f7ff] font-semibold">{t('academySection')}</h2>
                  <div className="h-px flex-1 bg-gradient-to-r from-[#99f7ff]/45 to-transparent" />
                  <Link href="/creator/academy" className="shrink-0 text-nx-10 text-[#a9abb5] hover:text-[#99f7ff]">{t('academyViewAll')}</Link>
                </div>
            ...
            <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-16 items-center justify-around gap-0.5 border-t border-white/10 bg-slate-950/90 px-1 backdrop-blur-md md:hidden">
              <MobileItem href="/creator" icon={LayoutDashboard} label={t('mobileHome')} active={isHubActive(pathname, '/creator')} />
              <MobileItem href="/creator/campaigns" icon={Megaphone} label={t('mobileCampaigns')} active={isHubActive(pathname, '/creator/campaigns')} />
              <Link
                ...
                aria-label={tNav('academy')}
              >
                <Plus className="h-5 w-5 text-[#005f64]" />
              </Link>
              <MobileItem href="/creator/deal-room" icon={Handshake} label={t('mobileDeals')} active={isHubActive(pathname, '/creator/deal-room')} />
              <MobileItem
                href="/creator/settings/notifications"
                icon={Bell}
                label={t('mobileAlerts')}
                active={pathname.startsWith('/creator/settings')}
              />
              <MobileItem href="/creator/profile" icon={User} label={t('mobileProfile')} active={isHubActive(pathname, '/creator/profile')} />
            </nav>
          </>
        )}
      </RoleLayoutShell>
...
function CalendarGrid({
  ...
  todayLabel: string
  noteLabel: string
  formatTasksScheduled: (n: number) => string
}) {
  ...
  return (
    <div className="grid grid-cols-7 gap-px overflow-hidden rounded-lg border border-white/5 bg-white/5 [contain:paint] isolate [transform:translateZ(0)]">
      {dayLabels.map((day, dayIdx) => (
        <div
          key={day}
          className={`bg-[#1b2029] text-center text-nx-8 ${isWeek ? 'p-2' : 'flex min-h-10 items-center justify-center p-1.5'} ${dayIdx >= 5 ? 'text-[#d873ff]' : 'text-white'
            }`}
        >
          {day}
        </div>
        ...
        >
          ...
          {isWeek && !cell.muted && getTaskCountForDate(cell.date, cell.muted) > 0 ? (
            <div className="mt-2 rounded border border-white/8 bg-white/[0.03] px-2 py-1 text-nx-9 leading-relaxed text-slate-300">
              {formatTasksScheduled(getTaskCountForDate(cell.date, cell.muted))}
            </div>
          ) : null}
        </button>
        )
