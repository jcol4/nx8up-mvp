...
import { linkYouTubeAccount, unlinkYouTubeAccount } from '@/app/creator/profile/_actions'
...
export default function YouTubeConnect({ initial }: YouTubeConnectProps) {
  ...
  const handleLink = async (formData: FormData) => {
    setIsLinking(true)
    setError('')
    const res = await linkYouTubeAccount(formData)
    if (res.error) {
      setError(res.error)
      setIsLinking(false)
    } else if (res.success && res.youtube) {
      setYoutube({
        handle: res.youtube.handle,
        channel_name: res.youtube.channel_name,
        subscribers: res.youtube.subscribers,
        avg_views: res.youtube.avg_views,
        top_categories: res.youtube.top_categories,
        synced_at: new Date(),
      })
      setShowForm(false)
      setIsLinking(false)
      ...
      <div className="yt-card">
        ...
        {youtube.handle ? (
          ...
          <>
            <div className="yt-profile">
              <div className="yt-avatar-placeholder">
                <svg width="20" height="16" viewBox="0 0 16 12" fill="currentColor">
                  <path d="M15.666 1.877A2.013 2.013 0 0 0 14.254.452C13.013.116 8 .116 8 .116s-5.013 0-6.254.336A2.013 2.013 0 0 0 .334 1.877C0 3.13 0 5.75 0 5.75s0 2.62.334 3.873A2.013 2.013 0 0 0 1.746 11.048C2.987 11.384 8 11.384 8 11.384s5.013 0 6.254-.336a2.013 2.013 0 0 0 1.412-1.425C16 8.37 16 16 5.75s0-2.62-.334-3.873ZM6.386 8.25V3.25L10.545 5.75 6.386 8.25Z"/>
                </svg>
              </div>

              <div className="yt-info">
                <div className="yt-display-name">
                  {youtube.channel_name ?? youtube.handle}
                </div>
            ...
            <p className="yt-approx-note">
              ⚠ Subscriber count may be rounded by YouTube for large channels
            </p>

            <div className="yt-actions">
              <button
                type="button"
                className="yt-btn yt-btn--unlink"
              ...
              {youtube.synced_at && (
                <span className="yt-synced">
                  Synced {new Date(youtube.synced_at).toLocaleDateString()}
                </span>
              )}
            </div>
          </>
          ...
          <>
            <form action={handleLink}>
              <div className="yt-form">
                ...
                <button
                  type="submit"
                  className="yt-btn yt-btn--submit"
                  disabled={isLinking}
                >
                ...
                <button
                  type="button"
                  className="yt-btn yt-btn--cancel"
          ...
          <div className="yt-empty">
            <svg width="32" height="24" viewBox="0 0 16 12" fill="rgba(255,0,0,0.2)">
              <path d="M15.666 1.877A2.013 2.013 0 0 0 14.254.452C13.013.116 8 .116 8 .116s-5.013 0-6.254.336A2.013 2.013 0 0 0 .334 1.877C0 3.13 0 5.75 0 5.75s0 2.62.334 3.873A2.013 2.013 0 0 0 1.746 11.048C2.987 11.384 8 11.384 8 11.384s5.013 0 6.254-.336a2.013 2.013 0 0 0 1.412-1.425C16 8.37 16 5.75 16 5.75s0-2.62-.334-3.873ZM6.386 8.25V3.25L10.545 5.75 6.386 8.25Z"/>
            </svg>
            <div className="yt-empty-title">Connect YouTube</div>
            <p className="yt-empty-sub">
              Link your YouTube channel so sponsors can see your subscriber count, average views, and content categories.
            </p>
            <button
              type="button"
              className="yt-btn yt-btn--connect"
